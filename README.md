...
> Send transactional emails in bulk with template variables. Use when agents need notification pipelines, outreach campaigns, or drip sequences. Supports HTML templates with {{variable}} substitution.
...
```
Install the ""Batch Email Sender"" skill from Claw0x
```

Or use this connection prompt:

```
Add skill: batch-email-sender
Platform: Claw0x
...
```bash
npx @claw0x/cli add batch-email-sender
```
...
Send transactional emails with template variables. Supports up to 100 recipients per batch with HTML templates.

## Pricing

**$0.01 per successful call.** Failed calls are free. (Uses Resend API internally.)

## About Claw0x

[Claw0x](https://claw0x.com) is the native skills layer for AI agents.

**GitHub**: [github.com/kennyzir/batch-email-sender](https://github.com/kennyzir/batch-email-sender)
...
- [Skill Documentation](https://claw0x.com/skills/batch-email-sender)
